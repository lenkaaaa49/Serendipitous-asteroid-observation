...
def importing_data(Option,password,Obs_date,Special_id1,Vertex1,Vertex2,Vertex3,Vertex4):
    ...
    if z==None:
        ...
        sql11= "UPDATE INPUT_Table SET Status='UPDATED', Time_imported_into_Database=NOW() where Special_id='{0}'".format(Special_id)
        #print (sql11)
        try:
        ...
        for x in range(0,len(f)):
            ...
            if JPL_SPKID[x] in result:
               ...
               try:
                    ...
                    db.rollback()
               #update Status in the input table to know if that line has been run already     
               sql11= "UPDATE INPUT_Table SET Status='UPDATED', Time_imported_into_Database=NOW() where Special_id='{0}'".format(tab_col[16])
               #print (sql11)
               try:
                    # Execute the SQL command
                    cursor.execute(sql11)
